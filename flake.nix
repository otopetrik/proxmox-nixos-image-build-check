{
  inputs = {
    nixpkgs = {
      ...
      url = "github:otopetrik/nixpkgs?ref=proxmox-image-uefi";
    };
  ...
  outputs = { self, nixpkgs, nixos-generators }: {
    ...
    packages.x86_64-linux = let
      ...
      bios-grub = nixos-generators.nixosGenerate {
        ...
        format = "proxmox";
        modules = [ ./common.nix ./bios-grub.nix saveRev ];
      };
      bios-grub-gpt = nixos-generators.nixosGenerate {
        pkgs = nixpkgs.legacyPackages.x86_64-linux;
        format = "proxmox";
        modules = [ ./common.nix ./bios-grub-gpt.nix saveRev ];
      };
      bios-hybrid-grub = nixos-generators.nixosGenerate {
        pkgs = nixpkgs.legacyPackages.x86_64-linux;
        format = "proxmox";
        modules = [ ./common.nix ./bios-hybrid-grub.nix saveRev ];
      };
      efi-grub = nixos-generators.nixosGenerate {
        pkgs = nixpkgs.legacyPackages.x86_64-linux;
        format = "proxmox";
        modules = [ ./common.nix ./efi-grub.nix saveRev ];
      };
      efi-hybrid-grub = nixos-generators.nixosGenerate {
        pkgs = nixpkgs.legacyPackages.x86_64-linux;
        format = "proxmox";
        modules = [ ./common.nix ./efi-hybrid-grub.nix saveRev ];
      };
      efi-systemd-boot = nixos-generators.nixosGenerate {
        pkgs = nixpkgs.legacyPackages.x86_64-linux;
        format = "proxmox";
        modules = [ ./common.nix ./efi-systemd-boot.nix saveRev ];
      };
      proxmox-images = with import nixpkgs { system = "x86_64-linux"; };
        stdenv.mkDerivation {
          name = "proxmox-images";
          src = self;
          buildInputs = [
